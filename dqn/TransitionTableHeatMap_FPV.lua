...
function trans:__init(args)
    ...
    self.heatmap_type = args.heatmap_network_type or 1
    ...
    if self.heatmap_type == 1 or self.heatmap_type == 2then
            s_size = self.stateDim * (histLen + histLen)
            self.heatmap = torch.ByteTensor(self.maxSize, self.stateDim):fill(0)
    else if self.heatmap_type == 3 then
            s_size = self.stateDim * (histLen + 1 )
            self.heatmap = torch.ByteTensor(self.maxSize / 4, self.stateDim ):fill(0)
    else
        print('Heatmap type not defined')
        exit()
    end
    ...
    assert(s_size, 's_size cannot be nil')
...
function trans:concatFrames(index, use_recent)
    if use_recent then
        s, t, hm = self.recent_s, self.recent_t, self.recent_heatmap
    else
        s, t, hm = self.s, self.t, self.heatmap
    end

    local fullstate = s[1].new()
    if self.heatmap_type == 1 or self.heatmap_type == 2 then
            fullstate:resize(self.histLen + self.histLen, unpack(s[1]:size():totable()))
    else if self.heatmap_type == 3
            fullstate:resize(self.histLen + 1, unpack(s[1]:size():totable()))
    end
    ...
    for i=self.histLen-1,1,-1 do
        ...
        if zero_out then
            ...
            if self.heatmap_type == 1 then
                fullstate[mapping[i]]:zero()
                fullstate[mapping[i]+ 1]:zero()
            else if self.heatmap_type == 2 then 
                fullstate[i]:zero()
                fullstate[i+ 4]:zero()
            else if self.heatmap_type == 3 then
                fullstate[i]:zero()
    ...
    for i=episode_start,self.histLen do
        ...
        if self.heatmap_type == 1 then
            ...
            fullstate[mapping[i]]:copy(s[index+self.histIndices[i]-1])
            fullstate[mapping[i] + 1]:copy(hm[index+self.histIndices[i]-1])

        else if self.heatmap_type == 2 then 
                
            fullstate[i]:copy(s[index+self.histIndices[i]-1])
            fullstate[i + 4]:copy(hm[index+self.histIndices[i]-1])

        else if self.heatmap_type == 3 then

            fullstate[i]:copy(s[index+self.histIndices[i]-1])
            fullstate[5]:copy(hm[ math.ceil( (index+self.histIndices[i]-1)  / 4  )])
        end
...
function trans:add(s, heatmap, a, r, term)
    ...
    if self.heatmap_type == 1 or self.heatmap_type == 2 then
        self.heatmap[self.insertIndex] = heatmap:clone():float():mul(255)
    else if self.heatmap_type == 3 then
        self.heatmap[math.ceil(self.insertIndex / 4)] = heatmap:clone():float(255)
    end
...
function trans:add_recent_state(s,heatmap, term)
    local s = s:clone():float():mul(255):byte()
    local heat_map = heatmap:clone():float():mul(255):byte()
    ...
    if #self.recent_s == 0 then
        for i=1,self.recentMemSize do
            ...
            --adding heatmap to the recent heatmaps
            table.insert(self.recent_heatmap, heat_map:clone():zero())
        end
    end

    table.insert(self.recent_s, s)
    table.insert(self.recent_heatmap, heat_map)
    ...
    if #self.recent_s > self.recentMemSize then
        ...
        table.remove(self.recent_heatmap, 1)
    end
end
...
function trans:read(file)
    ...
    print('transition table reading')
    hhsd()
